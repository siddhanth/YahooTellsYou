...
public class InfoBox {
	...
	public InfoBox(String[] potAns, String[] query, String[] queryEnt) {
		...
		/*
		 * System.out.println("quesKeywords"); for(String temp:quesKeywords){
		 * System.out.println(temp); }
		 */

	}

	public void parseInfoBox(String info, int call, String ansEntity)
			throws Exception {
	...
	public void parseTextBox(String text, int call, String ansEntity)
			throws Exception {
		if (call == 1) {
			// in query entity pages check for potential ans entities
			for (String key : scoreMap.keySet()) {
	...
	public static String readURL(String url) throws Exception {
		...
		if (obj3.keySet().contains("revisions")) {
			...
			JSONArray obj4 = (JSONArray) parser.parse(obj3.get("revisions")
					.toString());
	...
	public void update(String key, int call, String ansEnt) throws Exception {
		if (ansEnt != null) {
			...
			if (stemmedAnsEnt.compareTo(stemmedKey) != 0) {
				...
				String info = null;
				String text = null;
				info = readURL("http://en.wikipedia.org/w/api.php?action=query&prop=revisions&rvprop=content&format=json&titles="
						+ key + "&rvsection=0");
				text = readURL("http://en.wikipedia.org/w/api.php?action=query&prop=revisions&titles="
						+ key + "&rvprop=content&format=json");
