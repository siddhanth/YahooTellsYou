...
public class CountEntities {
	...
	CountEntities(HashMap<String, Double> ans2, HashMap<String, String> eumap,
			String param1, HashMap<String, String> param2) {
		queryent = param2;
		urlEntityMap = eumap;
		answerScoreMap = ans2;
	...
	// This function creates a boolean array corressponding to each entity
	// showing its presence in a answer
	...
	public HashMap<String, Double> contains() {
		...
		for (String key : urlEntityMap.keySet())
			for (String ans : answerScoreMap.keySet()) {
				if ((ans.split(urlEntityMap.get(key)).length - 1) > 0
						|| (ans.split(key).length - 1) > 0)
					if (urlCountMap.keySet().contains(key))
						urlCountMap.put(key, urlCountMap.get(key)
								+ answerScoreMap.get(ans));
					else
	 ...
	 * String[] ansArray;
	 * 
	 * // delimiter String delimiter = "~"; ansArray = answers.split(delimiter);
	 * 
	 * for(int i = 0; i < ansArray.length ; i++){ for (String key :
	 * urlEntityMap.keySet()) for(int j = 0 ; j < urlEntityMap.get(key).size();
	 * j++){ if(ansArray[i].contains(urlEntityMap.get(key).get(j))){
 ...
 * temp.put("rajendra prasad", "rajendra prasad"); temp.put("gavaskar",
 * "gavaskar"); temp.put("Gavaskar", "Gavaskar"); CountEntities c = new
 * CountEntities(ans,temp); HashMap<String, Integer> t1 = c.contains();
 * System.out.println(t1.get("rajendra prasad"));
 * System.out.println(t1.get("gavaskar"));
 * System.out.println(t1.get("Gavaskar")); }
 */
