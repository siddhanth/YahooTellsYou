...
public class freebaseReader {
	...
	public static String readMQL(String entity) throws Exception {
		...
		try {
			 ...
			 * 
			 * JSONObject
			 * obj1=(JSONObject)parser.parse(obj.get("q").toString()); JSONArray
			 * obj2=(JSONArray)parser.parse(obj1.get("messages").toString());
			 * JSONObject obj3=(JSONObject)parser.parse(obj2.get(0).toString());
			 * JSONObject
			 * obj4=(JSONObject)parser.parse(obj3.get("info").toString());
			 * 
			 * JSONArray
			 * obj5=(JSONArray)parser.parse(obj4.get("result").toString());
			 * 
			 * JSONObject obj6=(JSONObject)parser.parse(obj5.get(0).toString());
			 * 
			 * JSONArray
			 * obj7=(JSONArray)parser.parse(obj6.get("type").toString()); return
			 * obj7.get(1).toString();
			 */
			Pattern p = Pattern.compile("common(.*)$");

			Matcher matcher = p.matcher(sb.toString().replaceAll(
					"[^a-zA-Z0-9,]+", " "));
	...
	public HashMap<String, String> category(ArrayList<String> param1)
			throws Exception {
	...
	public static void main(String args[]) throws Exception {
		 ...
		 * PasswordAuthentication ("siddhanth", "14101989".toCharArray()); } });
		 */
		...
		/*
		 * temp.add("abdul kalam"); temp.add("pranab mukherjee");
		 * temp.add("india"); temp.add("car"); temp.add("scooter");
		 * temp.add("bugatti veyron"); temp.add("bill gates");
		 * temp.add("carlos slim"); temp.add("Larry ellison");
