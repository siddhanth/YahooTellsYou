...
public class entityFilter {
	...
	public static HashMap<String, String> run(String query,
			HashMap<String, String> entitySet) {
		...
		if (answerType != null) {
			...
			for (String key : entityCateg.keySet()) {
				if (entityCateg.get(key) != null
						&& entityCateg.get(key).contains(answerType)) {
			...
			for (String key : moreEntities.keySet()) {
				if (moreEntities.get(key) != null
						&& moreEntities.get(key).contains(answerType)) {
